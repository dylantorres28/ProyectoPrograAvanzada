...
const Tracks = () => {
    ...
    const trackId = state?.trackId; // Obtener el artistId del estado
    ...
    useEffect(() => {
        if (!trackId) return; // Si no hay trackId, no hacer la llamada
        ...
        const fetchArtistDetails = async () => {
            try {
                ...
                const responseTracks = await fetch(
                    `https://spotify-downloader9.p.rapidapi.com/tracks?ids=${trackId}`,
                    {
                        method: "GET",
                        headers: {
