...
const Payment_Gateway = () => {
    ...
    return (
        <div className="w-full min-h-screen text-white bg-gradient-to-br from-[#01283A] to-black overflow-x-hidden p-4">
            ...
            <div className="mb-4">
                ...
                <div className="flex space-x-4">
                    <button
                        ...
                        className={`px-4 py-2 rounded hover:bg-blue-600 ${selectedPaymentMethod === 'Tarjeta Credito' ? 'bg-blue-600' : 'bg-gray-600'}`}
                    >
                        Tarjeta de Crédito
                    </button>
                    <button
                        onClick={() => handlePaymentMethodSelect('Paypal')}
                        className={`px-4 py-2 rounded hover:bg-blue-600 ${selectedPaymentMethod === 'Paypal' ? 'bg-blue-600' : 'bg-gray-600'}`}
                    >
                        PayPal
                    </button>
                    <button
                        onClick={() => handlePaymentMethodSelect('Transferencia Bancaria')}
                        className={`px-4 py-2 rounded hover:bg-blue-600 ${selectedPaymentMethod === 'Transferencia Bancaria' ? 'bg-blue-600' : 'bg-gray-600'}`}
                    >
                        Transferencia
                    </button>
                </div>
            ...
            {/* Invoice Details */}
            <div className="bg-[#1A1A1A] rounded-lg p-6 mb-4 bg-white/5 ">
                <div className="mb-4">
                    <h3 className="text-xl mb-3 font-semibold">Detalles de Compra</h3>
                    {factura.Detalle && factura.Detalle.length > 0 ? (
                        <table className="w-full text-center border-b border-gray-600">
                            <thead>
                                <tr className="bg-gray-700 ">
                                    <th className="p-2">#</th>{/*Columna*/}
                            ...
                            <tbody>
                                {factura.Detalle.map((item, index) => (
                                    <tr key={index}>
                                        ...
                                        <td className=" p-2">{item.Nombre_Artistico}</td>
                                        <td className=" p-2">{item.Nombre_Album}</td>
                                        <td className=" p-2">{item.Nombre_Cancion}</td>
                                        <td className=" p-2">{item.Cantidad_Facturada}</td>
                                        <td className=" p-2">${item.Total_Cancion.toFixed(2)}</td>
            ...
            {selectedPaymentMethod === 'Tarjeta Credito' && (
                <div className="mb-4">
                    <label htmlFor="cvv" className="block text-white mb-2">Codigo Tarjeta</label>
                    <input
                        type="text"
                        id="cvv"
                        maxLength="3"
            ...
            <button
                ...
                className={`w-full p-3 rounded  ${selectedPaymentMethod
